...
Remotes are named the git way: `git mess remote add origin <hub>` stores the URL in your mess (as ordinary git remote config), after which `push`/`fetch`/`pull` need no remote argument at all — they default to `origin`. `git mess clone` sets `origin` automatically, so a cloned mess syncs bare-handed from the start. `git mess remote` lists what's configured; `remote remove <name>` drops one. Any command also still accepts an explicit remote name, path, or URL.
...
- **Diverged** (you both snapshotted) → a **3-way merge**: the common ancestor comes from `git merge-base`, and each file is merged with `git merge-file`. Non-overlapping edits combine cleanly into a merge version with both chains as parents. Overlapping edits produce standard conflict markers, which are **written to the file and recorded**, loudly flagged, and make `pull` exit nonzero — edit the file and `snapshot` to resolve. (Recording the conflicted state keeps everyone's graphs converged, so a half-resolved state can never fork the team; the tradeoff is that markers exist in history until resolved.)
